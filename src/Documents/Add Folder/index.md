# Add Folder

Create a new folder in Documents to organize files in. From [Documents](/documents/), click **Add Folder**.

<p><img src="../../images/documents/add-folder-form.png" alt="Add Folder form" class="border"></p>

**Name** | **Description**
:--- | ---
Name | The folder's internal name. Lowercase, letters/numbers/hyphens only.
Title | The folder's display title.
Description | An optional description, for organizational purposes.
Submit | Click **Submit** to create the folder.

Once created, click the folder's name to open it -- see [Document Overview](/documents/document-overview/) for everything you can do inside it, including adding more folders to build out a nested structure.
