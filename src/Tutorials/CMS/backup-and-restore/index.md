...
2. Navigate to **EC2** and click on your **new stack**.

3. Get the private IP address of your new server by navigating to your new EC2 instance and copying the **Private IPv4** address from the instance details.
...
4. Connect to your new server using your preferred **SSH client** or terminal.

5. Type the following command to switch to root user:
```
sudo bash
```
...
7. Run the restore command, replacing the IP address below with your current server's private IP:
```
./cms.sh restore 172.00.00.000
```

8. The system will provide you with a set of commands (typically 4 lines) to configure SSH and authorize the connection.

9. Copy the entire output exactly as shown — you'll need to paste and run these commands on your current server in the next step.

10. Connect to your current server again.
...
12. Paste and run the complete command provided in the previous step. This will configure SSH settings, restart the SSH service, and authorize your new server to access the current server.

13. Once complete, you can disconnect from the server.
